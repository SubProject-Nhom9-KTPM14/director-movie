...
//
//		Supplier<List<Movie>> flightsSupplier =
...
//	}
	...
	void timeoutExample() {
		...
		Supplier<List<Movie>> flightsSupplier = RateLimiter.decorateSupplier(limiter, () -> service.searchFlights(request));
		for (int i=0; i<3; i++) {
			try {
				System.out.println(flightsSupplier.get());
			}
	...
	void checkedExceptionExample() {
		...
		CheckedFunction0<List<Movie>> flights
	...
	void multipleLimits_2rps_40rpm_sequential() {
		...
		RateLimiter rpsLimiter = registry
				.rateLimiter("flightSearchService_rps", rpsConfig);
		RateLimiter rpmLimiter = registry
				.rateLimiter("flightSearchService_rpm", rpmConfig);


		MovieService service = new MovieService();
		SearchRequest request = new SearchRequest("NYC", "LAX", "07/31/2020");

		Supplier<List<Movie>> rpsLimitedSupplier = RateLimiter.decorateSupplier(rpsLimiter, () -> service.searchFlights(request));
		Supplier<List<Movie>> flightsSupplier = RateLimiter.decorateSupplier(rpmLimiter, rpsLimitedSupplier);
		for (int i=0; i<45; i++) {
			try {
				System.out.println(flightsSupplier.get());
			}
	...
	void changeLimitsExample() {
		...
		RateLimiterRegistry registry
				= RateLimiterRegistry.of(config);
		RateLimiter limiter
				= registry.rateLimiter("flightSearchService");
	...
	void retryAndRateLimit() {
		...
		Retry retry = retryRegistry
				.retry("rateLimitedFlightSearch", retryConfig);
		retry.getEventPublisher()
				.onRetry(e -> System.out.println(e.toString()));
		retry.getEventPublisher()
				.onRetry(e -> System.out.println(e.toString()));

		retry.getEventPublisher()
				.onSuccess(e -> System.out.println(e.toString()));

		Supplier<List<Movie>> rateLimitedFlightsSupplier =
				RateLimiter.decorateSupplier(rateLimiter, () -> service.searchFlights(request));
		Supplier<List<Movie>> retryingFlightsSupplier =
	...
	private void limit_1rps_concurrent() {
		...
		SearchRequest request =
			new SearchRequest("NYC", "LAX", "07/31/2020");

		Supplier<List<Movie>> flightsSupplier = RateLimiter.decorateSupplier(limiter,
				() -> service.searchFlights(request));

		CompletableFuture.supplyAsync(flightsSupplier)
				.thenAccept(System.out::println);
		CompletableFuture.supplyAsync(flightsSupplier)
				.whenComplete((flights, e) -> {
	...
	void rateLimiterEvents() {
		...
		limiter.getEventPublisher().onSuccess(
				e -> System.out.println(e.toString()));
		limiter.getEventPublisher().onFailure(
				e -> System.out.println(e.toString()));
	...
	void rateLimiterMetrics() {
		...
		SearchRequest request = new SearchRequest("NYC", "LAX", "07/31/2020");
		Supplier<List<Movie>> flightsSupplier = RateLimiter.decorateSupplier(retry, () -> service.searchFlights(request));
...
//		}
		MovieApplication examples = new MovieApplication();
